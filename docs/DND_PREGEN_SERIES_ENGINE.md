...
The subclass segment is mandatory. It prevents two subclasses of the same class and level from sharing a slot, record, or picker value.
...
The series blueprint stores stable identity, edition, class, subclass, species, background, and sources. Each level package stores final level-specific statistics, proficiencies, attacks, resources, spellcasting, features, equipment, and print-review state.
...
- `DndPregenLibrary.tsx`: picker state and selected record display.
- `DndPregenValidationPanel.tsx`: validation presentation.

## Current Coverage

- 480 public build slots remain represented across both editions.
- 40 Fighter / Champion sheets remain released and printable.
- 440 remaining entries stay visibly marked as Blueprints.
- The next ladder cannot collide with Fighter / Champion because class and subclass are both part of identity.
...
2. Define one reviewed level series for every remaining public class/subclass path.
3. Generate and validate all twenty records per series.
4. Add catalog-wide uniqueness and completeness checks.
5. Load verified pregens into Player Workspace and live combat.
