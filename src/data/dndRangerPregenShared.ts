...
const rangerSpellPool: Array<[number, string]> = [
...
export const rangerSpells2014 = (level: number): string[] => rangerSpellPool
  .filter(([unlock]) => level >= unlock)
  .slice(0, rangerKnownCounts2014[level - 1])
  .map(([, spell]) => spell);

export const rangerSpells2024 = (level: number): string[] => rangerSpellPool
  .filter(([unlock]) => level >= unlock)
