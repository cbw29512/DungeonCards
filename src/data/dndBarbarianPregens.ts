...
const classFeatures2014 = (level: number): string[] => [
  `Rage: ${rageUses2014(level) === "unlimited" ? "unlimited uses" : `${rageUses2014(level)} uses per Long Rest`}; +${rageDamage(level)} Strength-melee damage; Advantage on Strength checks and saves; Resistance to bludgeoning, piercing, and slashing damage`,
...
const make2014 = (level: number): DndCharacterRecord => {
  ...
  if (!slot) throw new Error(`Missing 2014 Barbarian / Berserker build slot at level ${level}.`);
...
const make2024 = (level: number): DndCharacterRecord => {
  ...
  if (!slot) throw new Error(`Missing 2024 Barbarian / Berserker build slot at level ${level}.`);
