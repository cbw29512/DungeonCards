...
describe("Barbarian Berserker ready-to-play pregens", () => {
  ...
  it("requires every published Barbarian record to pass the hard readiness gate", () => {
  ...
  it("centralizes all released sheets without duplicate build slots", () => {
    ...
    expect(getDndReadyPregenRecord("srd-5.2.1-2024", "barbarian", 20)?.name).toBe("Torra Ashfang");
    expect(getDndReadyPregenRecord("srd-5.1-2014", "fighter", 20)?.name).toBe("Kara Stoneguard");
  });
});
