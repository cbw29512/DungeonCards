...
export const PlayableCardRuntimePanel = (props: Props) => (
  <article
    className={`playable-card-runtime${props.active ? " is-active" : ""}`}
    id={`playable-card-${props.instance.id}`}
    tabIndex={-1}
  >
    ...
    <section className="playable-card-runtime__controls" aria-label={`Runtime controls for ${props.definition.content.title}`}>
      ...
      <label><span>Custom name</span><input defaultValue={props.instance.customName ?? ""} key={`${props.instance.id}-name-${props.instance.customName ?? ""}`} onBlur={(event) => props.onUpdateText({ customName: event.target.value })} placeholder={props.definition.content.title} /></label>
      <label><span>Runtime notes</span><textarea defaultValue={props.instance.notes} key={`${props.instance.id}-notes-${props.instance.notes}`} onBlur={(event) => props.onUpdateText({ notes: event.target.value })} rows={3} /></label>
          ...
          {props.definition.resources.map((resource) => {
            ...
            return (
              <article key={resource.id}>
                ...
                <div><button disabled={unlimited || current <= 0} onClick={() => props.onAdjustResource(resource.id, -1)} type="button">−</button><button disabled={unlimited || atMaximum} onClick={() => props.onAdjustResource(resource.id, 1)} type="button">+</button><button onClick={() => props.onResetResource(resource.id)} type="button">Reset</button></div>
              </article>
            );
          })}
