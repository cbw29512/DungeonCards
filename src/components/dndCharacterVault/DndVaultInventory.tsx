  ...
  <div className="character-vault__panel-grid">
    ...
    <section className="character-vault__card">
      ...
      {profile?.magicItems.map((item) => {
        const remaining = savedState?.itemChargeState[item.id];
        ...
        return (
          <article className="character-vault__magic-item" key={item.id}>
            <header>
              ...
              <span>{item.rarity.replace("-", " ")}{item.requiresAttunement ? ` · ${attuned ? "Attuned" : "Not attuned"}` : ""}</span>
            </header>
            <p>{item.effectSummary}</p>
            <small>
              {item.synergyNote}
              {item.maximumCharges ? ` · ${remaining ?? item.maximumCharges} / ${item.maximumCharges} charges` : ""}
