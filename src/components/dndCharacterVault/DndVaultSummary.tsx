...
export const DndVaultSummary = ({
  ...
  const hitPoints = savedState
    ? `${savedState.currentHitPoints} / ${record.maximumHitPoints}`
    : record.maximumHitPoints;
  ...
  return (
    <>
      <section className="character-vault__vitals" aria-label="Core character statistics">
        ...
        <article>
          <span>Hit Points</span><strong>{hitPoints}</strong>
          {savedState && savedState.temporaryHitPoints > 0 && <small>+{savedState.temporaryHitPoints} temporary</small>}
        </article>
        ...
        {savedState && (
          <article>
            ...
            <strong>{savedState.deathSaveSuccesses} success · {savedState.deathSaveFailures} failure</strong>
          </article>
        )}
      </section>
